...
/*!
  * Speed Reading v1.2
  * Copyright 2023 The Speed Reading Author (https://github.com/da-ns)
  * Licensed under MIT (https://github.com/twbs/bootstrap/blob/main/LICENSE)
  */
!function(t) {
    let e = !0
    ...
    function p(o) {
        ...
        l = n[o],
        s = 0,
        t("#progress").prop("max", l.length),
        e && d()
    }
    function g() {
        let o;
        r ? o = l[Math.floor(Math.random() * (l.length - 1))] : (o = l[s],
        t("#progress").val(s + 1),
        t("#progressOutput").text(s + 1)),
        i && (o = '<div class="rotating text-uppercase">' + (o = Array.from(o).join('</div><div class="rotating text-lowercase">')) + "</div>"),
        a.html(o),
        r || (s = s == l.length - 1 ? 0 : s + 1),
        e && d()
    }
    function d() {
        f(),
    ...
    a = t("#word"),
    t("#complexity").on("input", function e(o) {
        let n = t(o.target).val();
        t("#complexitySetting").text(n),
        f(),
        p(n)
    }),
    t("#random").on("change", function o(n) {
        (r = t(n.target).is(":checked")) ? t("#progressFormGroup").hide() : (s = 0,
        t("#progressOutput").text(s + 1),
        t("#progressFormGroup").show()),
        e && d()
    }),
    t("#progress").on("input", function o(n) {
        s = parseInt(t(n.target).val()),
        t("#progressOutput").text(s + 1),
        e && d()
    }),
    t(".time").on("input", function o(n) {
        c = t(n.target).val(),
        t(".time").val(c),
        t("#timeSetting").text(c),
        c *= 1e3,
        e && d()
    }),
    t("#rotating").on("change", function e(o) {
        i = t(o.target).is(":checked")
    }),
    t("#inverted").on("change", function e(o) {
        (u = t(o.target).is(":checked")) ? a.addClass("inverted") : a.removeClass("inverted")
    }),
    t("#playButton").on("click", function o() {
        e = !0,
        t("#playButton").hide(),
        t("#pauseButton").show(),
        d()
    }),
    t("#pauseButton").on("click", function o() {
        e = !1,
        t("#pauseButton").hide(),
        t("#playButton").show(),
        f()
    }),
    t("#fullscreenButton").on("click", function e() {
        t("#display").addClass("fullscreen"),
        t("#toolbar").addClass("fullscreen"),
        t("#fullscreenButton").hide(),
        t("#noFullscreenButton").show(),
        t("html").get(0).scrollIntoView(),
        t("html").css("overflow", "hidden")
    }),
    t("#noFullscreenButton").on("click", function e() {
        t("#display").removeClass("fullscreen"),
        t("#toolbar").removeClass("fullscreen"),
        t("#noFullscreenButton").hide(),
        t("#fullscreenButton").show(),
        t("#display").get(0).scrollIntoView(),
        t("html").css("overflow", "auto")
    }),
    t.getJSON("data/words-groups.min.json", function(t) {
        for (let e = 0; e < t.groups.length; e++) {
            let o = t.groups[e];
            n[o.complexity] = o.words
        }
        p(),
        d()
    })
